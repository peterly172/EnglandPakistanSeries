...
SELECT match_id, t.name AS team, p.name AS players, runs, innings,
...
SELECT match_id, p.name AS player, runs, balls, SUM(balls/6.0) AS OVERS
FROM matchruns
JOIN players AS p
ON matchruns.player_id = p.id
WHERE runs IS NOT NULL
GROUP BY match_id, p.name, runs, balls
ORDER BY SUM(runs/6.0) DESC

--Run ranges by a player (Mohammad Rizwan)
WITH bins AS (
